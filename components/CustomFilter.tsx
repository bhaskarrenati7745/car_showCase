            ...
            <Listbox value={selected} onChange={(e) => { setSelected(e); handleUpdateParams(e) }}>
                <div className='relative z-10 w-fit'>
                    <Listbox.Button className='custom-filter__btn'>
                        ...
                        <Image src='/chevron-up-down.svg' className='ml-4 object-contain' height={20} width={20} alt='chevron up down' />
                    </Listbox.Button>
                        ...
                        <Listbox.Options className='custom-filter__options'>
                            ...
                            {options.map((option) => (
                                <Listbox.Option key={option.title} value={option} className={({ active }) => `relative cursor-default select-none py-2 px-4 ${active ? 'bg-primary-blue text-white' : 'text-gray-900'}`}>
                                    {({ selected }) => (
                                        <span className={`truncate block ${selected ? 'font-medium' : 'font-normal'}`}>{option.title}</span>
                                    )}
                                </Listbox.Option>
                            ))}
                        </Listbox.Options>
